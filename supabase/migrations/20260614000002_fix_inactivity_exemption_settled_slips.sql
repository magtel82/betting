-- Fix inactivity-fee exemption to credit settled slips covering the fee date.
--
-- Bug: activity check #2 only exempted players with an OPEN/LOCKED slip on a
-- match that day. With auto-settlement, a pre-placed slip flips to won/lost the
-- moment the match finishes — before the next-morning fee job runs. The player
-- was then wrongly fined despite having bet on that day's match.
--
-- Fix: count ANY non-cancelled slip (open/locked/won/lost/void) covering a match
-- on the fee date as activity. Only a player-cancelled slip no longer counts.
--
-- Full replacement of apply_inactivity_fee — only activity check #2 changed.
