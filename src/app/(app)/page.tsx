...
type NextMatch = {
  id: string;
  scheduled_at: string;
  home_team: { name: string; short_name: string; flag_code: string | null } | null;
  away_team: { name: string; short_name: string; flag_code: string | null } | null;
...
export default async function DashboardPage() {
  ...
  const [slipsRes, matchesRes, marketsRes, betsRes, todayMatchesRes] = await Promise.all([
    ...
    supabase
      .from("matches")
      .select(
        "id, scheduled_at, home_team:teams!matches_home_team_id_fkey(name, short_name, flag_code), away_team:teams!matches_away_team_id_fkey(name, short_name, flag_code), odds:match_odds(home_odds, draw_odds, away_odds)"
      )
      .eq("status", "scheduled")
      .order("scheduled_at")
      .limit(6),
    ...
    <>
      ...
      <div className="mx-auto max-w-lg space-y-5 px-4 py-5">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/bet"
            className="flex h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-bold text-white shadow-sm transition-colors hover:bg-[var(--primary-600)] active:bg-[var(--primary-600)]"
          >
          ...
          <Link
            ...
            className="flex h-12 items-center justify-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 text-sm font-semibold text-gray-600 shadow-sm transition-colors hover:bg-gray-50 active:bg-gray-100"
            title="Din spelkalender"
          >
            📅
          </Link>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-sm font-semibold text-gray-900">
                      <Flag code={m.home_team?.flag_code} className="text-base" />{" "}
                      {m.home_team?.name ?? "?"}
                      <span className="mx-1.5 text-gray-300">vs</span>
                      <Flag code={m.away_team?.flag_code} className="text-base" />{" "}
                      {m.away_team?.name ?? "?"}
                    </p>
                    {o && (
